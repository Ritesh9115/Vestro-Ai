...
export default function SignupPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast.success('Welcome to Vestro AI!')
      navigate('/me')
    } catch (err) {
      const detail = err.response?.data?.details
      if (detail) setErrors(detail.map((d) => d.message))
    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#FBFBF8' }}>
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontFamily: "'Fraunces',serif", fontSize: '1.75rem', fontWeight: 600, color: '#0F211A', margin: '20px 0 8px' }}>Create your account</h1>
          <p style={{ color: '#5B6B63', fontSize: '0.9rem' }}>Start researching smarter today</p>
        </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #E5E8E2', borderRadius: 16, padding: 28, boxShadow: '0 4px 24px rgba(15,33,26,0.06)' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {/* Name */}
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#0F211A', marginBottom: 6 }}>Full Name</label>
              <div style={{ position: 'relative' }}>
                <User size={16} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#9AA69F' }} />
                <input
                  type="text" placeholder="Your name" value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  required style={{ width: '100%', padding: '10px 12px 10px 36px', border: '1px solid #E5E8E2', borderRadius: 10, fontSize: '0.9rem', outline: 'none', fontFamily: 'Inter,sans-serif', boxSizing: 'border-box' }}
                />
              </div>
            </div>
            ...
            {/* Password */}
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#0F211A', marginBottom: 6 }}>Password</label>
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  required style={{ width: '100%', padding: '10px 40px 10px 36px', border: '1px solid #E5E8E2', borderRadius: 10, fontSize: '0.9rem', outline: 'none', fontFamily: 'Inter,sans-serif', boxSizing: 'border-box' }}
                />
            ...
            {/* Errors */}
            {errors.length > 0 && (
              <div style={{ background: '#FBEAE8', border: '1px solid #F3D4D0', borderRadius: 8, padding: '10px 14px' }}>
                {errors.map((e, i) => <p key={i} style={{ margin: '2px 0', color: '#C8443A', fontSize: '0.8rem' }}>• {e}</p>)}
              </div>
            )}
        ...
        <p style={{ textAlign: 'center', marginTop: 20, color: '#5B6B63', fontSize: '0.875rem' }}>
          ...
          <Link to="/login" style={{ color: '#0E8F5B', fontWeight: 600, textDecoration: 'none' }}>Sign in</Link>
        </p>
      </motion.div>
    </div>
  )
